...
import { Film, RefreshCw, Star, Wand2 } from 'lucide-react'
import { post } from '../api/client'

export default function SceneStudioModal({ scene, project, locations, onClose, onReferenceSelected }) {
  const [generating, setGenerating] = useState(false)
  const [candidates, setCandidates] = useState([])
  const [selecting, setSelecting] = useState(null)
  const [canonicalPath, setCanonicalPath] = useState(scene.reference_image_path ?? null)
  ...
  const handleGenerate = async () => {
    ...
    setCandidates([])
    try {
      const data = await post(`/scenes/${scene.id}/generate-reference`)
  ...
  const handleSelect = async (relativePath) => {
    ...
    setError('')
    try {
      const updated = await post(`/scenes/${scene.id}/select-reference`, {
      ...
      <div className="modal-pixel max-w-3xl w-full">
        ...
        <div className="modal-header">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="heading-pixel-sm text-accent-400">SCENE STUDIO</span>
          ...
          <div className="flex gap-6">
            ...
            <div className="flex-shrink-0 flex flex-col items-center gap-3">
              ...
              <div
                ...
                style={{ width: '192px', height: '108px', boxShadow: '3px 3px 0 0 #000', background: '#1c1c38' }}
              >
                {canonicalPath ? (
                  <img
              ...
              {canonicalPath && (
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-pixel text-px-green" style={{ fontSize: '7px' }}>SEEDING I2V</span>
                </div>
              )}
            </div>

            {/* Right — controls + context + candidates */}
            <div className="flex-1 flex flex-col gap-4">

              {/* Scene visual description */}
              <div>
                <div className="label-pixel mb-1">VISUAL PROMPT</div>
                <p className="text-retro text-zinc-400 bg-zinc-900 border border-zinc-700 p-2"
                  style={{ fontSize: '15px', lineHeight: '1.5' }}>
                  {scene.visual || (
                    <span className="text-zinc-600 italic">No visual description — add one in scene settings first.</span>
                  )}
                </p>
              </div>
              ...
              <div className="bg-zinc-900 border border-zinc-700 p-3 space-y-1.5">
                <div className="label-pixel mb-1">INJECTED INTO PROMPT</div>
                {location && (
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  disabled={generating || !scene.visual}
                ...
                {generating && (
                  <span className="text-retro text-zinc-500" style={{ fontSize: '15px' }}>
                    Running FLUX schnell... (~15s per candidate)
                  </span>
                )}
              </div>
              ...
              {!scene.visual && (
                <p className="text-retro text-amber-500" style={{ fontSize: '15px' }}>
                  ⚠ Add a visual description to the scene first.
                </p>
              )}
                    ...
                    {displayCandidates.map((relPath, i) => {
                      ...
                      return (
                        <button
                          ...
                          style={{ width: '160px', height: '90px', boxShadow: '2px 2px 0 0 #000' }}
                          title={isCanonical ? 'Current canonical reference' : 'Click to use as seed'}
                        >
                          <img
