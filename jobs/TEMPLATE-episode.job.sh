...
step "probe" \
     "$PY scripts/probe_shot.py $SERIES --episode $EP --auto"

# The job stops here on purpose. Nothing automated can tell you the shot
# shows the right person in the right place in the right style; that needs
# an eye. Look at the probe clips, then re-run the same job with the gate
# lifted and jobctl skips straight to the render:
#
#     PROBE_REVIEWED=1 scripts/jobctl start jobs/my-episode.job.sh
#
step "probe-reviewed" \
     "test \"\${PROBE_REVIEWED:-0}\" = 1 || {
        echo '';
        echo '  ── STOP ─────────────────────────────────────────────';
        echo '  Three probe shots are rendered. Look at them before';
        echo '  committing 2.5 hours to the episode:';
        echo '';
        echo \"    ls -t ComfyUI/output/video/$SERIES/probe_* | head\";
        echo '';
        echo '  Then resume with:';
        echo \"    PROBE_REVIEWED=1 scripts/jobctl start \$(basename \$0)\";
        echo '  ─────────────────────────────────────────────────────';
        exit 1; }"
