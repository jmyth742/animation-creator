...
for L in lines:
    ...
    opening = np.clip(jaw - 0.6 * close, 0, 1)
    vis = np.zeros(n_frames, np.int64)
    vis[opening >= 0.06] = 1
    vis[opening >= 0.16] = 2
    vis[opening >= 0.30] = 3
    vis[opening >= 0.48] = 4
    vis[(rnd > 0.30) & (opening >= 0.06)] = 5
    env = np.clip(opening * 1.6, 0, 1).astype(np.float32)
    ...
    plot(jaw, (200, 60, 40), 70); plot(rnd, (40, 120, 200), 140); plot(smile, (40, 160, 80), 140); plot(blink, (120, 40, 160), 200)
    dr.text((10, 2), f"l{i} {L['who']}  red=jawOpen  blue=round  green=smile  purple=blink  bars=viseme column", fill=(30, 30, 30))
