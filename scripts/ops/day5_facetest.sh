...
for CAST in mv cg; do D=$W/ft_${CAST}; rm -rf $D; mkdir -p $D; $B -b --factory-startup $R/film_nine_waterfalls_$CAST.blend --python scripts/blender3d/film.py -- $D "$C" "$T" $L $FM $FM static < /dev/null > $D.log 2>&1 &
...
ffmpeg -v error -y -i $(ls $W/ft_mv/*.png) -i $(ls $W/ft_cg/*.png) -i $(ls $W/ft4_mv/*.png) -i $(ls $W/ft4_cg/*.png) -filter_complex "[0][1]hstack[a];[2][3]hstack[b];[a][b]vstack" $R/day5_cast_facing.png && log "day5_cast_facing.png (meet: mv|cg over close: mv|cg)"
log "FACETEST DONE"
