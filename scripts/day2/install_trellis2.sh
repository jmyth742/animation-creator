...
$P -m pip install -q $X/nvdiffrast --no-build-isolation || log "nvdiffrast FAILED"
[ -d $X/nvdiffrec ] || git clone -q -b renderutils https://github.com/JeffreyXiang/nvdiffrec.git $X/nvdiffrec
$P -m pip install -q $X/nvdiffrec --no-build-isolation || log "nvdiffrec FAILED"
[ -d $X/CuMesh ] || git clone -q --recursive https://github.com/JeffreyXiang/CuMesh.git $X/CuMesh
$P -m pip install -q $X/CuMesh --no-build-isolation || log "CuMesh FAILED"
[ -d $X/FlexGEMM ] || git clone -q --recursive https://github.com/JeffreyXiang/FlexGEMM.git $X/FlexGEMM
$P -m pip install -q $X/FlexGEMM --no-build-isolation || log "FlexGEMM FAILED"
rm -rf $X/o-voxel; cp -r $R/o-voxel $X/o-voxel
$P -m pip install -q $X/o-voxel --no-build-isolation || log "o-voxel FAILED"
...
$E/bin/huggingface-cli download microsoft/TRELLIS.2-4B > /workspace/loopwork/t2_dl.log 2>&1 && log "weights cached" || log "weights download FAILED"
